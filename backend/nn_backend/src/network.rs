...
impl Network {
    ...
    pub fn train(
        ...
        for i in 1..=epochs {
            if epochs < 100 || i % (epochs / 100) == 0 {
                ic_cdk::api::print(format!("Epoch {} of {}", i, epochs));
            }
            for j in 0..inputs.len() {
                ic_cdk::api::print(format!("inputs[j]: {:?}", inputs[j]));
    ...
    pub fn single_train(&mut self, inputs: Vec<f64>, targets: Vec<f64>) -> Result<(), String> {
