...
if [ -n "$val" ]; then
  ...
  do
    ...
    if [ -n "$exist_canister" ]; then
      ...
      shell_path="./backend/${dir}/create_cindidfile.sh"
...
if [ -n "$val" ]; then
  ...
  echo "NFID_ORIGIN=https://nfid.one" >> .env.development
  echo "DFX_NETWORK=${target_key_list[1]}" >> .env.development
  echo "NEXT_PUBLIC_APPLICATION_NAME=ReunionDemo" >> .env.development
fi
# 4 <<<<<<<<<<<<<<<<<<<<<<<